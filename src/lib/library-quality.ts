...
export const qualityDocuments: Record<string, InstrumentDocument> = {
  formControl: libraryDoc("formControl", {
    ...
    outputs: [
      { id: "observedSpan", label: "Observed {formType} extrema span", family: "length", defaultUnit: "mm", expression: "(measuredMaximum-measuredMinimum)*0.001" },
  ...
  mmc: libraryDoc("mmc", {
    ...
    outputs: [
      { id: "bonus", label: "Available bonus tolerance", family: "length", defaultUnit: "mm", expression: "((2*lookup(isHole, featureType)-1)*(actualSize-mmcSize))*0.001" },
      { id: "totalPosition", label: "Total position tolerance", family: "length", defaultUnit: "mm", expression: "(positionTolerance+(2*lookup(isHole, featureType)-1)*(actualSize-mmcSize))*0.001" },
      { id: "virtualCondition", label: "Simplified virtual condition", family: "length", defaultUnit: "mm", expression: "(mmcSize+(1-2*lookup(isHole, featureType))*positionTolerance)*0.001" },
    ],
  ...
  orientationControl: libraryDoc("orientationControl", {
    ...
    outputs: [
      { id: "variation", label: "Observed orientation-reading variation", family: "length", defaultUnit: "mm", expression: "(maximumReading-minimumReading)*0.001" },
      { id: "ratio", label: "Observed variation / stated tolerance", defaultUnit: "%", expression: "(maximumReading-minimumReading)/tolerance*100" },
      { id: "difference", label: "Stated tolerance − observed variation", family: "length", defaultUnit: "mm", expression: "(tolerance-(maximumReading-minimumReading))*0.001" },
    ],
  ...
  profileRunout: libraryDoc("profileRunout", {
    ...
    outputs: [
      { id: "variation", label: "Observed indicator variation", family: "length", defaultUnit: "mm", expression: "(maximumReading-minimumReading)*0.001" },
      { id: "ratio", label: "Observed variation / stated tolerance", defaultUnit: "%", expression: "(maximumReading-minimumReading)/tolerance*100" },
      { id: "difference", label: "Stated tolerance − observed variation", family: "length", defaultUnit: "mm", expression: "(tolerance-(maximumReading-minimumReading))*0.001" },
    ],
    formula: "observed variation = rmax − rmin · ratio = observed variation / stated tolerance",
  ...
  toleranceStack: libraryDoc("toleranceStack", {
    ...
    outputs: [
      { id: "worstCase", label: "Worst-case stack tolerance", family: "length", defaultUnit: "mm", expression: "(abs(t1)+abs(t2)+abs(t3)+abs(t4)+abs(t5)+abs(t6))*0.001" },
      { id: "rss", label: "RSS stack tolerance", family: "length", defaultUnit: "mm", expression: "(sqrt(t1^2+t2^2+t3^2+t4^2+t5^2+t6^2))*0.001" },
      { id: "worstMin", label: "Worst-case lower result", family: "length", defaultUnit: "mm", expression: "(nominal-(abs(t1)+abs(t2)+abs(t3)+abs(t4)+abs(t5)+abs(t6)))*0.001" },
      { id: "worstMax", label: "Worst-case upper result", family: "length", defaultUnit: "mm", expression: "(nominal+(abs(t1)+abs(t2)+abs(t3)+abs(t4)+abs(t5)+abs(t6)))*0.001" },
    ],
