...
export const mechanicsDocuments: Record<string, InstrumentDocument> = {
  ballScrewLife: libraryDoc("ballScrewLife", {
    ...
    outputs: [
      { id: "nominalLifeRevolutions", label: "Nominal life", family: "angle", defaultUnit: "rev", expression: "(((((dynamicRating)/(axialLoad)))^(3)*1e6))*6.283185307179586" },
      { id: "idealTravelDistance", label: "Ideal travel distance", family: "length", defaultUnit: "km", expression: "((((((dynamicRating)/(axialLoad)))^(3)*1e6)*(lead)/1e6))*1000" },
  ...
  ballScrewSizing: libraryDoc("ballScrewSizing", {
    ...
    outputs: [
      ...
      { id: "power", label: "Mechanical drive power", family: "power", defaultUnit: "kW", expression: "((((axialForce)*(lead)/1000/(2*pi*((efficiency)/100)))*2*pi*(speed)/60/1000))*1000" },
    ],
  ...
  beltAxis: libraryDoc("beltAxis", {
    ...
    outputs: [
      ...
      { id: "power", label: "Mechanical linear power", family: "power", defaultUnit: "kW", expression: "((((mass)*9.80665*(friction))*(linearSpeed)/1000))*1000" },
    ],
  ...
  conveyorLine: libraryDoc("conveyorLine", {
    ...
    outputs: [
      ...
      { id: "lineSpeed", label: "Declared line speed", family: "speed", defaultUnit: "m/min", expression: "(lineSpeed)*0.016666666666666666", when: "lookup(isRate, solveFor)" },
      ...
      { id: "lineSpeed", label: "Literal line speed", family: "speed", defaultUnit: "m/min", expression: "(requestedRate*productPitch/1000)*0.016666666666666666", when: "1-lookup(isRate, solveFor)" },
  ...
  deflectionCheck: libraryDoc("deflectionCheck", {
    ...
    outputs: [
      { id: "referenceDeflection", label: "Declared-reference deflection", family: "length", defaultUnit: "mm", expression: "(((span)/(referenceDenominator)))*0.001" },
      ...
      { id: "arithmeticDifference", label: "Reference minus declared deflection", family: "length", defaultUnit: "mm", expression: "((((span)/(referenceDenominator))-(declaredDeflection)))*0.001" },
    ],
  ...
  driveTrain: libraryDoc("driveTrain", {
    ...
    outputs: [
      ...
      { id: "inputPower", label: "Literal input power", family: "power", defaultUnit: "kW", expression: "(((inputTorque)*(inputSpeed)*(2*pi/60))/1000)*1000" },
      { id: "outputPower", label: "Literal output power", family: "power", defaultUnit: "kW", expression: "((((inputTorque)*(inputSpeed)*(2*pi/60))*((stage1Efficiency)*(stage2Efficiency)*(stage3Efficiency)))/1000)*1000" },
      { id: "arithmeticLoss", label: "Declared-efficiency arithmetic loss", family: "power", defaultUnit: "kW", expression: "((((inputTorque)*(inputSpeed)*(2*pi/60))-(((inputTorque)*(inputSpeed)*(2*pi/60))*((stage1Efficiency)*(stage2Efficiency)*(stage3Efficiency))))/1000)*1000" },
    ],
  ...
  gearToothStress: libraryDoc("gearToothStress", {
    ...
    outputs: [
      ...
      { id: "loadedSection", label: "Lewis-type loaded section factor", family: "area", defaultUnit: "mm²", expression: "(faceWidth*module*formFactor)*1e-06" },
  ...
  jointSeparation: libraryDoc("jointSeparation", {
    ...
    outputs: [
      { id: "loadFraction", label: "Bolt load fraction C", family: "dimensionless", defaultUnit: "%", expression: "(boltStiffness/(boltStiffness+memberStiffness)*100)*0.01" },
      { id: "boltLoadIncrease", label: "External-load share carried by bolt", family: "force", defaultUnit: "kN", expression: "(boltStiffness/(boltStiffness+memberStiffness)*externalLoad)*1000" },
      { id: "residualClamp", label: "Residual clamp force", family: "force", defaultUnit: "kN", expression: "(preload-(1-boltStiffness/(boltStiffness+memberStiffness))*externalLoad)*1000" },
      { id: "separationLoad", label: "Ideal separation-load threshold", family: "force", defaultUnit: "kN", expression: "(preload/(1-boltStiffness/(boltStiffness+memberStiffness)))*1000" },
      { id: "separationReserve", label: "Threshold minus stated external load", family: "force", defaultUnit: "kN", expression: "(preload/(1-boltStiffness/(boltStiffness+memberStiffness))-externalLoad)*1000" },
    ],
  ...
  linearGuideLife: libraryDoc("linearGuideLife", {
    ...
    outputs: [
      { id: "nominalLifeKm", label: "Nominal travel life", family: "length", defaultUnit: "km", expression: "(lookup(lifeRef, rollingType)*(dynamicRating/calculatedLoad)^lookup(lifeExp, rollingType))*1000" },
  ...
  mohrCircle: libraryDoc("mohrCircle", {
    ...
    outputs: [
      ...
      { id: "principalAngle", label: "One principal-plane orientation", family: "angle", defaultUnit: "°", expression: "(0.5*atan2(2*tauXY, sigmaX-sigmaY)*180/pi)*0.017453292519943295" },
      { id: "doublePrincipalAngle", label: "Mohr-circle double angle to principal plane", family: "angle", defaultUnit: "°", expression: "(atan2(2*tauXY, sigmaX-sigmaY)*180/pi)*0.017453292519943295" },
      { id: "doubleMaxShearAngle", label: "Mohr-circle double angle to max-shear plane", family: "angle", defaultUnit: "°", expression: "(atan2(2*tauXY, sigmaX-sigmaY)*180/pi+90)*0.017453292519943295" },
    ],
  ...
  pinStress: libraryDoc("pinStress", {
    ...
    outputs: [
      ...
      { id: "shearArea", label: "One nominal pin shear area", family: "area", defaultUnit: "mm²", expression: "(pi*pinDiameter^2/4)*1e-06" },
      ...
      { id: "projectedBearingArea", label: "Projected plate bearing area per pin", family: "area", defaultUnit: "mm²", expression: "(pinDiameter*plateThickness)*1e-06" },
  ...
  pressFit: libraryDoc("pressFit", {
    ...
    outputs: [
      { id: "interference", label: "Diametral interference", family: "length", defaultUnit: "mm", expression: "(((shaftDiameter)-(holeDiameter)))*0.001" },
  ...
  rackPinion: libraryDoc("rackPinion", {
    ...
    outputs: [
      ...
      { id: "power", label: "Mechanical linear power", family: "power", defaultUnit: "kW", expression: "((((mass)*9.80665*(friction)+(mass)*(acceleration)+(externalForce))*(linearSpeed)/1000))*1000" },
    ],
  ...
  sCurveProfile: libraryDoc("sCurveProfile", {
    ...
    outputs: [
      { id: "peakSpeed", label: "Profile peak speed", family: "speed", defaultUnit: "mm/s", expression: "(min(topSpeed, sqrt(distance*averageAcceleration)))*0.001" },
  ...
  shaftDesign: libraryDoc("shaftDesign", {
    ...
    outputs: [
      { id: "minimumDiameter", label: "Minimum torsion-only diameter", family: "length", defaultUnit: "mm", expression: "((16*(torque*1000)/(pi*allowableShear))^(1/3))*0.001" },
      ...
      { id: "centralDeflection", label: "Central-load deflection", family: "length", defaultUnit: "mm", expression: "(centerLoad*length^3/(48*youngModulus*1000*(pi*shaftDiameter^4/64)))*0.001" },
  ...
  threadTensileArea: libraryDoc("threadTensileArea", {
    ...
    outputs: [
      { id: "effectiveDiameter", label: "Empirical tensile-area diameter", family: "length", defaultUnit: "mm", expression: "(((majorDiameter)-0.938194*(pitch)))*0.001" },
      { id: "tensileArea", label: "External thread tensile stress area", family: "area", defaultUnit: "mm²", expression: "((0.7854*(((majorDiameter)-0.938194*(pitch)))^(2)))*1e-06" },
    ],
  ...
  torqueSpeedDuty: libraryDoc("torqueSpeedDuty", {
    ...
    outputs: [
      ...
      { id: "shaftPower", label: "Duty-point shaft power", family: "power", defaultUnit: "kW", expression: "((((loadTorque)+((inertia)*((speedChange)*(2*pi/60)/(accelerationTime))))*(targetSpeed)*2*pi/60/1000))*1000" },
