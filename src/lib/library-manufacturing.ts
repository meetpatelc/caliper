...
export const manufacturingDocuments: Record<string, InstrumentDocument> = {
  ...
  cuttingParameters: libraryDoc("cuttingParameters", {
    ...
    outputs: [
      ...
      { id: "feedRate", label: "Table feed rate", family: "speed", defaultUnit: "mm/min", expression: "(chipLoad*teeth*1000*cuttingSpeed/(pi*diameter))*1.6666666666666667e-05" },
      ...
      { id: "power", label: "Specific-force power estimate", family: "power", defaultUnit: "kW", expression: "(axialDepth*radialWidth*chipLoad*teeth*1000*cuttingSpeed/(pi*diameter)*specificForce/(60e6*efficiency/100))*1000" },
    ],
  ...
  cuttingPower: libraryDoc("cuttingPower", {
    ...
    outputs: [
      { id: "cuttingPower", label: "Ideal cutting power", family: "power", defaultUnit: "kW", expression: "(depth*feed*cuttingSpeed*specificForce/60000)*1000" },
      { id: "machinePower", label: "Declared-efficiency machine input", family: "power", defaultUnit: "kW", expression: "(depth*feed*cuttingSpeed*specificForce/60000/(efficiency/100))*1000" },
    ],
  ...
  drillPointDepth: libraryDoc("drillPointDepth", {
    ...
    outputs: [
      { id: "pointDepth", label: "Geometric drill-point depth", family: "length", defaultUnit: "mm", expression: "(diameter/2/tan((includedAngle/2)*pi/180))*0.001" },
      { id: "programmedDepth", label: "Programmed depth for declared full diameter", family: "length", defaultUnit: "mm", expression: "(fullDiameterDepth+diameter/2/tan((includedAngle/2)*pi/180))*0.001" },
    ],
  ...
  drillingTime: libraryDoc("drillingTime", {
    ...
    outputs: [
      { id: "cuttingSpeed", label: "Peripheral cutting speed", family: "speed", defaultUnit: "m/min", expression: "(pi*diameter*rpm/1000)*0.016666666666666666" },
      { id: "feedRate", label: "Spindle feed rate", family: "speed", defaultUnit: "mm/min", expression: "(feedPerRev*rpm)*1.6666666666666667e-05" },
      { id: "timeMinutes", label: "Nominal cutting time", family: "time", defaultUnit: "s", expression: "depth*holes/(feedPerRev*rpm)*60" },
      { id: "distance", label: "Total programmed cutting depth", family: "length", defaultUnit: "mm", expression: "(depth*holes)*0.001" },
    ],
  ...
  filletWeld: libraryDoc("filletWeld", {
    ...
    outputs: [
      { id: "throat", label: "Effective throat thickness", family: "length", defaultUnit: "mm", expression: "(0.707*legSize)*0.001" },
      { id: "effectiveArea", label: "Effective throat area", family: "area", defaultUnit: "mm²", expression: "(0.707*legSize*weldLength*weldLines)*1e-06" },
      ...
      { id: "requiredLeg", label: "Required leg at stated allowable", family: "length", defaultUnit: "mm", expression: "(directForce/(0.707*weldLength*allowableShear*weldLines))*0.001" },
  ...
  fixtureClamping: libraryDoc("fixtureClamping", {
    ...
    outputs: [
      { id: "designHorizontalForce", label: "Multiplier-adjusted horizontal force", family: "force", defaultUnit: "N", expression: "machiningForce*serviceMultiplier" },
      { id: "requiredNormalForce", label: "Friction-only required normal force", family: "force", defaultUnit: "N", expression: "machiningForce*serviceMultiplier/friction" },
      { id: "frictionCapacity", label: "Friction capacity at required normal force", family: "force", defaultUnit: "N", expression: "machiningForce*serviceMultiplier" },
    ],
  ...
  machiningTimeBudget: libraryDoc("machiningTimeBudget", {
    ...
    outputs: [
      ...
      { id: "totalTimeSeconds", label: "Literal declared total time", family: "time", defaultUnit: "s", expression: "(((cuttingLength)/(feedRate))+(nonCutAllowance))*60" },
    ],
  ...
  pickPlaceCycle: libraryDoc("pickPlaceCycle", {
    ...
    outputs: [
      { id: "cycleTime", label: "Declared repeated cycle time", family: "time", defaultUnit: "s", expression: "outboundTime+inboundTime+pickDwell+placeDwell+auxiliaryTime" },
  ...
  sheetBendAllowance: libraryDoc("sheetBendAllowance", {
    ...
    outputs: [
      { id: "bendAllowance", label: "Literal single-bend allowance", family: "length", defaultUnit: "mm", expression: "((((bendAngle)*pi/180)*((insideRadius)+(kFactor)*(thickness))))*0.001" },
      { id: "bendDeduction", label: "Literal single-bend deduction", family: "length", defaultUnit: "mm", expression: "((2*((insideRadius)+(thickness))*tan(((bendAngle)*pi/180)/2)-(((bendAngle)*pi/180)*(((insideRadius))+(kFactor)*((thickness))))))*0.001" },
      { id: "neutralAxisRadius", label: "Literal neutral-axis radius", family: "length", defaultUnit: "mm", expression: "((insideRadius)+(kFactor)*(thickness))*0.001" },
      { id: "bendAngle", label: "Declared bend angle", family: "angle", defaultUnit: "°", expression: "((bendAngle))*0.017453292519943295" },
    ],
  ...
  sheetMetalBend: libraryDoc("sheetMetalBend", {
    ...
    outputs: [
      { id: "bendAllowance", label: "Bend allowance", family: "length", defaultUnit: "mm", expression: "((angle*pi/180)*(insideRadius+kFactor*thickness))*0.001" },
      { id: "outsideSetback", label: "Outside setback", family: "length", defaultUnit: "mm", expression: "(tan((angle*pi/180)/2)*(insideRadius+thickness))*0.001" },
      { id: "bendDeduction", label: "Bend deduction", family: "length", defaultUnit: "mm", expression: "(2*tan((angle*pi/180)/2)*(insideRadius+thickness)-(angle*pi/180)*(insideRadius+kFactor*thickness))*0.001" },
      { id: "flatLength", label: "One-bend flat-length estimate", family: "length", defaultUnit: "mm", expression: "(flange1+flange2-(2*tan((angle*pi/180)/2)*(insideRadius+thickness)-(angle*pi/180)*(insideRadius+kFactor*thickness)))*0.001" },
    ],
  ...
  tappingTorque: libraryDoc("tappingTorque", {
    ...
    outputs: [
      { id: "torque", label: "User-coefficient first-estimate tapping torque", family: "torque", defaultUnit: "N·m", expression: "((torqueCoefficient)*((threadDiameter))^(3)*(engagementFactor))" },
      { id: "mechanicalPower", label: "Ideal mechanical spindle power", family: "power", defaultUnit: "kW", expression: "((((torqueCoefficient)*((threadDiameter))^(3)*(engagementFactor))*2*pi*(spindleSpeed)/60/1000))*1000" },
  ...
  taylorToolLife: libraryDoc("taylorToolLife", {
    ...
    methodChoice: "mode",
    outputs: [
      { id: "cuttingSpeed", label: "Declared cutting speed", family: "speed", defaultUnit: "m/min", expression: "(cuttingSpeed)*0.016666666666666666", when: "lookup(isLife, mode)" },
      ...
      { id: "cuttingSpeed", label: "Taylor-relation cutting speed", family: "speed", defaultUnit: "m/min", expression: "(taylorConstant/toolLife^exponent)*0.016666666666666666", when: "1-lookup(isLife, mode)" },
  ...
  threadDesign: libraryDoc("threadDesign", {
    ...
    outputs: [
      { id: "targetTapDrill", label: "Target tap drill at stated engagement", family: "length", defaultUnit: "mm", expression: "(((majorDiameter)-(pitch)*(engagementPercent)/100))*0.001" },
      { id: "fullEngagementTapDrill", label: "Basic major-minus-pitch drill", family: "length", defaultUnit: "mm", expression: "(((majorDiameter)-(pitch)))*0.001" },
      { id: "engagedThreads", label: "Engaged thread pitches", defaultUnit: "—", expression: "((engagementLength)/(pitch))" },
      { id: "rootShearArea", label: "Reviewed root-shear area", family: "area", defaultUnit: "mm²", expression: "(((pi*(threadsPerInch)*((engagementLength)/25.4)*(externalMajorMinimum)*(1/(2*(threadsPerInch))+0.57735*((externalMajorMinimum)-(internalPitchMaximum))))*645.16))*1e-06" },
      { id: "pullOutCapacity", label: "User-property thread-stripping capacity", family: "force", defaultUnit: "N", expression: "(((pi*(threadsPerInch)*((engagementLength)/25.4)*(externalMajorMinimum)*(1/(2*(threadsPerInch))+0.57735*((externalMajorMinimum)-(internalPitchMaximum))))*645.16)*(allowableShear))" },
  ...
  threadMachiningTime: libraryDoc("threadMachiningTime", {
    ...
    outputs: [
      { id: "feedRate", label: "Declared pitch-feed rate", family: "speed", defaultUnit: "mm/min", expression: "(((pitch)*(spindleSpeed)))*1.6666666666666667e-05" },
      { id: "cuttingTimePerPass", label: "Declared travel time per pass", family: "time", defaultUnit: "s", expression: "((travelLength)/((pitch)*(spindleSpeed))*60)" },
      { id: "totalCuttingTime", label: "Declared repeated travel time", family: "time", defaultUnit: "s", expression: "(((travelLength)/((pitch)*(spindleSpeed))*60)*(passCount))" },
      { id: "totalTime", label: "Declared travel + reversal time", family: "time", defaultUnit: "s", expression: "((((travelLength)/((pitch)*(spindleSpeed))*60)*((passCount)))+(reversalTime)*(passCount))" },
    ],
  ...
  weldGroup: libraryDoc("weldGroup", {
    ...
    outputs: [
      { id: "totalLineLength", label: "Total effective line length", family: "length", defaultUnit: "mm", expression: "((2*(lineLength)))*0.001" },
      ...
      { id: "endpointRadius", label: "Farthest endpoint radius", family: "length", defaultUnit: "mm", expression: "((sqrt(((centerSpacing)/2)^2+((lineLength)/2)^2)))*0.001" },
      { id: "directLineLoad", label: "Direct shear line load", family: "stiffness", defaultUnit: "N/mm", expression: "(((directForce)/(2*(lineLength))))*1000" },
      { id: "torsionalEndpointLineLoad", label: "Endpoint torsional line load", family: "stiffness", defaultUnit: "N/mm", expression: "(((torsionalMoment*1000)*(sqrt(((centerSpacing)/2)^2+((lineLength)/2)^2))/((lineLength)*((centerSpacing))^(2)/2+((lineLength))^(3)/6)))*1000" },
      { id: "quadratureLineLoad", label: "Quadrature line-load magnitude", family: "stiffness", defaultUnit: "N/mm", expression: "((sqrt((((directForce)/(2*(lineLength))))^2+(((torsionalMoment*1000)*(sqrt(((centerSpacing)/2)^2+((lineLength)/2)^2))/((lineLength)*((centerSpacing))^(2)/2+((lineLength))^(3)/6)))^2)))*1000" },
    ],
