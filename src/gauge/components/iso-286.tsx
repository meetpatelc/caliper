...
import { officialBySlug } from "@/gauge/lib/catalog";
          ...
          result.ok ? (
            <>
              <div className="grid gap-1.5">
                <QuantityName label="Fit" />
                <p className="font-mono text-3xl font-medium tabular-nums tracking-tight">{fitLabel(result.fit.kind)}</p>
                <p className="font-mono text-xs text-muted">
                  {result.fit.holeClass}/{result.fit.shaftClass}
                </p>
              </div>
              <div className="grid gap-1.5">
                <QuantityName label="Maximum clearance" symbol="cmax" />
                <span className="flex items-center gap-2">
                  <p className="min-w-0 flex-1 font-mono text-3xl font-medium tabular-nums tracking-tight">{mm(result.fit.cmax)}</p>
                  <UnitBadge>mm</UnitBadge>
                </span>
              </div>
              <div className="grid gap-1.5">
                <QuantityName label="Maximum interference" symbol="imax" />
                <span className="flex items-center gap-2">
                  <p className="min-w-0 flex-1 font-mono text-3xl font-medium tabular-nums tracking-tight">{mm(result.fit.imax)}</p>
                  <UnitBadge>mm</UnitBadge>
                </span>
              </div>
