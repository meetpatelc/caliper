...
import type { AnyCalculator, WorkshopCalculator } from "@/gauge/lib/calculator-types";
import { isPackedCalculator } from "@/gauge/lib/calculator-types";
import { relatedCalculators } from "@/gauge/lib/catalog";
import { defaultFieldState, evaluateCalculator, retargetField, type FieldState } from "@/gauge/lib/evaluate";
            ...
            {result.ok ? (
              <ul className="grid gap-4">
                {result.outputs.map((output) => {
                  ...
                  return (
                    <li key={output.id} className="grid gap-1.5">
                      <QuantityName label={output.label} symbol={output.symbol} />
                      <span className="flex items-center gap-2">
                        <p className="min-w-0 flex-1 font-mono text-3xl font-medium tabular-nums tracking-tight">
                          {output.display}
                        </p>
                        {family ? (
                          <UnitSelect
